...
public class PipeBakedModel implements IBakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand) {
        ...
        if (state != null) {
            if (state.getValue(BlockPipe.UP)) {
                quads.addAll(handle_side_u.get().getQuads(state, side, rand));
            }

            if (state.getValue(BlockPipe.DOWN)) {
                quads.addAll(handle_side_d.get().getQuads(state, side, rand));
            }

            if (state.getValue(BlockPipe.EAST)) {
                quads.addAll(handle_side_e.get().getQuads(state, side, rand));
            }

            if (state.getValue(BlockPipe.WEST)) {
                quads.addAll(handle_side_w.get().getQuads(state, side, rand));
            }

            if (state.getValue(BlockPipe.NORTH)) {
                quads.addAll(handle_side_n.get().getQuads(state, side, rand));
            }

            if (state.getValue(BlockPipe.SOUTH)) {
                quads.addAll(handle_side_s.get().getQuads(state, side, rand));
            }
    ...
    public static class ModelLoader implements ICustomModelLoader {
        ...
        public boolean accepts(ResourceLocation modelLocation) {
            if (!modelLocation.getNamespace().equals(Everpipe.MODID))
                return false;
